        ...
        if (isset($_GET['action'])) {
          if ($_GET['action'] === 'confirm_email') {
            ...
            <div class="col-md-6 col-lg-5">
              <div class="confirm-container">
                ...
                <form action="">
                  ...
                  <button type="submit" name="action" value="unsubscribe" class="blc-btn btn btn-block">Unsubscribe</button>
                </form>
              </div>
            </div> 
            <?php
            ...
            if ($_GET['confirmed-email'] === $_GET['email']) {
              if (deleteEmail($conn, $_GET['email'])) {
                ?>
                <h1 class="text-success">Succefully Unsubscribed</h1>
                <?php
              } else {
                ?>
                <h1 class="text-danger">Failed to unsubscribe</h1>
                <?php
              }
            } else {
              ?>

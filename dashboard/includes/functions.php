...
function updateMaterial($conn, $data) {
  $stmt = mysqli_prepare($conn, "UPDATE materials SET title=?, author=?, pub_date=?, category=? WHERE code='" . $data['code'] ."'");
  mysqli_stmt_bind_param($stmt, 'ssss', $data['title'], $data['author'], $data['pub_date'], $data['category']);
...
function uploadVideo($file) {
  if ($file['error'] === 0) {
    $ext = explode('.', $file['name']);
    $newName = 'mat_' . $code . '.' . $ext[count($ext) - 1];
    $destination = '../uploads/videos/vid_' . $newName;
    $success = move_uploaded_file($file['tmp_name'], $destination);
    if ($success) {
      return $newName;
    } else {
      die('Error : Could\'t uploading new video ,error uploading video file ,try again!!!');
    }
  } else {
...
function uploadVideoPreview($file) {
    ...
    $ext = explode('.', $file['name']);
    $newName = 'mat_' . $code . '.' . $ext[count($ext) - 1];
    $destination = '../uploads/videos/vcap_' . $newName;
    $success = move_uploaded_file($file['tmp_name'], $destination);
    if ($success) {
      return $newName;
    } else {
